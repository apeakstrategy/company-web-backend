...
exports.notification = (inquiry, adminUrl) => ({
  ...
  html: layout("New website inquiry", `<p style="color:#d5c8c0"><strong>Reference:</strong> ${escapeHtml(inquiry.reference)}<br><strong>Name:</strong> ${escapeHtml(inquiry.name)}<br><strong>Email:</strong> ${escapeHtml(inquiry.email)}<br><strong>Subject:</strong> ${escapeHtml(inquiry.subject)}</p><div style="white-space:pre-wrap;line-height:1.7;border-top:1px solid rgba(255,255,255,.1);padding-top:18px">${escapeHtml(inquiry.message)}</div><p style="margin-top:24px"><a href="${escapeHtml(adminUrl)}" style="background:#E67E22;color:white;text-decoration:none;padding:12px 20px;border-radius:999px">Open in admin</a></p>`),
});
...
exports.reply = (inquiry, subject, message) => ({
  subject,
  text: message,
  html: layout(subject, `<p style="color:#d5c8c0">Hi ${escapeHtml(inquiry.name)},</p><div style="white-space:pre-wrap;line-height:1.7;color:#fff">${escapeHtml(message)}</div><p style="margin-top:24px;color:#a99588;font-size:12px">Regarding inquiry ${escapeHtml(inquiry.reference)}</p>`),
});
